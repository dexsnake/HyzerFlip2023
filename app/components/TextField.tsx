...
export const TextField = forwardRef(function TextField(props: TextFieldProps, ref: Ref<TextInput>) {
	...
	return (
		<TouchableOpacity activeOpacity={1} style={$containerStyles} onPress={focusInput} accessibilityState={{ disabled }}>
			{!!(label || labelTx) && (
				<Text className="mb-2 text-sm font-medium text-gray-900" style={$labelStyles}>
					{label}
				</Text>
			)}

			<View
				className={twMerge('flex-row items-start border-b border-b-gray-300 overflow-hidden bg-white', isFocused ? 'border-flip-100' : 'border-b-gray-300')}
				style={$inputWrapperStyles}
			>
				{!!LeftAccessory && <LeftAccessory style={$leftAccessoryStyle} status={status} editable={!disabled} multiline={TextInputProps.multiline} />}

				<TextInput
					ref={input}
					className="flex-1 self-stretch text-gray-800 text-base font-normal leading-5 h-6 mb-1 p-0"
					underlineColorAndroid={tw.transparent}
					textAlignVertical="top"
					selectionColor={tw.flip[100]}
					placeholder={placeholderContent}
					placeholderTextColor={tw.gray[400]}
					{...TextInputProps}
					editable={!disabled}
					style={$inputStyles}
				...
				{!!RightAccessory && <RightAccessory style={$rightAccessoryStyle} status={status} editable={!disabled} multiline={TextInputProps.multiline} />}
			</View>
